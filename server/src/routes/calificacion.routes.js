...
const { protect, authorize, autorizarAccesoEstudiante } = require('../middleware/auth');
...
const { verificarPeriodoAbierto, verificarVentanaRecuperacion } = require('../middleware/academic');
...
router.get('/estudiante/:estudianteId/anio/:anioAcademicoId', protect, reglas.estudianteId, reglas.anioAcademicoId, validar, autorizarAccesoEstudiante(), heavyQueryLimiter, controller.getBoletin);
...
router.post('/recuperacion', protect, verificarVentanaRecuperacion, writeLimiter, authorize(...PERMISOS.DOCENTE), controller.calcularRecuperacion);

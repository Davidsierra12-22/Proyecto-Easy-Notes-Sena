...
// --- Función Específica ---
const getByAnio = async (req, res) => {
    try {
        const data = await Model.find({ anioAcademicoId: req.params.anioAcademicoId });
        res.json({ ok: true, data, message: 'Grupos obtenidos por año académico' });
    } catch (error) {
        res.status(500).json({ ok: false, message: 'Error al obtener', error: error.message });
    }
};

module.exports = { getAll, getById, create, update, remove, getByAnio };